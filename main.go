...
func (t Training) distance() float64 { // c 31-33 my code
...
func (t Training) meanSpeed() float64 {
	...
	mnSpeed := (float64(t.Action) * t.LenStep / MInKm) / t.Duration.Hours()
	return mnSpeed
}
...
func (t Training) TrainingInfo() InfoMessage {
	return InfoMessage{
		...
		Distance:     t.distance(),   // Дистанция, пройденная за тренировку
		Speed:        t.meanSpeed(),  // Средняя скорость
		Calories:     t.Calories(),   // Калории, потраченные за тренировку
	}
}
...
func (r Running) Calories() float64 {
	// вставьте ваш код ниже
	RunCal := ((CaloriesMeanSpeedMultiplier*r.meanSpeed())/r.Duration.Hours() + CaloriesMeanSpeedShift) * r.Weight / MInKm * r.Duration.Hours() * MinInHours
	return RunCal
}
...
func (w Walking) Calories() float64 {
	...
	walkCal = ((CaloriesWeightMultiplier*w.Weight + (math.Pow(speedMps, 2)/heightM)*CaloriesSpeedHeightMultiplier*w.Weight) * w.Duration.Hours() * MinInHours)
	return walkCal
	//math.Pow использовать для возвышения в степень
}
...
func (s Swimming) meanSpeed() float64 {
	...
	mnSpeed := float64(s.LengthPool*s.CountPool) / MInKm / s.Duration.Hours()
	return mnSpeed
}
...
func (s Swimming) Calories() float64 {
	// вставьте ваш код ниже
	swimCal := (s.meanSpeed() + SwimmingCaloriesMeanSpeedShift) * SwimmingCaloriesWeightMultiplier * s.Weight * s.Duration.Hours()
	return swimCal
}
...
func (s Swimming) TrainingInfo() InfoMessage {
	...
	return InfoMessage{
		...
		Duration:     s.Duration,
		Distance:     s.distance(), // Вы можете использовать этот метод или свой, если требуется другой расчет
		Speed:        s.meanSpeed(),
		Calories:     s.Calories(),
	}
}
